...
| 9 | Package audit: GNOME session/bootstrap removed, survivors justified | Partial | exact `22105aa6` Voulage packet and six-package ownership audit pass; the [Ubuntu 26.04 local-pool closure](proof-notes/2026-08-17-ubuntu-resolute-local-pool-closure.md) installs the three session targets, and the [fresh exact-tuple QEMU proof](proof-notes/2026-08-17-exact-session-package-qemu-criterion-9.md) reaches a COSMIC login with GNOME target inactive; clean archive install and complete survivor scope remain open |
